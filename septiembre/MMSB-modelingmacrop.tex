...
\end{equation}
This is called the average effective contribution per woman parasite to the parasite reservoir %(in the form of eggs or larvae)
and can be obtained as in \cite{churcher2006density,lopez2022general}.
%is known as the average effective contribution per female to the parasite reservoir (in the form of eggs or larvae) 
%and we can calculate it as shown in  \cite{churcher2006density}.
}
...
%However only hosts with at least one female and one male parasites will effectively contribute to the parasite's reservoir by laying fertilized eggs or infective eggs. Therefore the mean fertilized egg production per host is 
...
%Finally, the dynamics for the mean parasite burden $m$ is obtained as follow. Parasites are taken from the environment at the rate 	$\beta N \ell $ and therefore the mean parasite burden increases at the rate  $\beta  N\ell/N=\beta\ell $. Parasites within the host die at the rate $\mu_p$ and hosts at the rate $\mu_h$ (killing all their parasites). Thus, the dynamics of $m$ is
...
%the average number of new parasite offsprings caused by one typical parasite, from one generation to the next.
%The basic reproductive number, R 0 , is defined as 
...
%and substituting (9) in equation (7) we obtain the following equation for the dynamics of m
...
%The bifurcation point is $(m^b, R_0^b)$ where 
...
%As shown in the next section, the dynamic system \eqref{model1} present a saddle-node bifurcation and therefore, for $R_the dynamic system \eqref{model1} present0 > R_0^b$ there are three equilibria (see Figure 2). 
\begin{itemize}
...
%One of the solutions is the stable endemic equilibrium which is an attractor for a range of va- lues of $R_0> R_0^b $ .

%The other solution is an unstable equilibrium and corresponds to a repulsor in the phase plane, that is, a barrier where values of m(t) above the unstable equili- brium are attracted towards the stable equilibrium and values of m(t) below the unstable equilibrium are attracted to the extinction equilibrium m ∗ = 0, which is the trivial solution of the equation (10) .





%As shown in the next section, the system undergoes a saddle-node bifurcation and therefore, for  $R_0> R_0^b $ there are three equilibria
%(see Figure \ref{f:phase}). 
%One of the solutions is the \textbf{stable}  endemic equilibrium %$\tilde m$ (\ref{meq}),  
%which is an attractor for a range of values of $R_0> R_0^b $ .
...
\begin{multline}
%{\scriptstyle	
	\frac{dm}{dt}=f(m^b,R_0^b)+(m-m^b)\frac{\partial f }{\partial m}\big\vert_{(m^b,R_0^b)}%\delta M
...
%For this, we assume that a host population, $H$, is divided into subpopulations or groups, $H_i$, that present different risks of infection (for example by, age groups, risk groups, environmental conditions, access to sanitation and hygiene, etc. 
...
%The dynamics of parasitic infection  for this type of population is described below	
%	
%	
%	
%In this section we will consider the more general and realistic case for a host population $H$. Unlike the homogeneous model presented in the previous section, here we present a model that accounts for host population heterogeneity (for example, age groups, risk groups, environmental conditions, ac051M33l.cess to sanitation  and  hygiene, etc. \cite{anderson1992infectious,anderson2014coverage,brooker2006contrasting,freeman2015associations,truscott2014modeling}), where subpopulations $H_i$  have different infection risks.
The dynamics of parasitic infection for the case of a heterogeneous population is described as follows
...
%the other parameters that detail correspond to each group $H_i$
\begin{itemize}
...
%\begin{equation}
%\begin{split}
%\dfrac{dm_{i}}{dt}=\beta_{i} \frac{\lambda_0\alpha}{ (\mu_{\ell}+\sum_j N_j \beta_j  ) }  {\color{red}
%\sum_j     \rho_{j} N_j  m_{j} F(m_{j})  - (\mu_h+\mu_p) m_{i}%\notag es para no tener numero en ecuacion{\color{red}{\color{red}
%\end{split}
%\end{equation}
...
The mean parasite burden $m$ of the host population %$H=\bigcup_i H_i$ 
is given by
\begin{equation}
m=\sum_i \pi_i m_{i} 
\end{equation}
...
%This is not an explicit expression of theof the group  equilibria $m_{i}^*$. Therefore, the equilibrium value can only be solved numerically.
An equilibrium condition for 
%the mean burdens of each subpopulation $H_{i}$ 
$m_{i}^*$
is given by
\begin{equation}%\label{eqequilibrio}
